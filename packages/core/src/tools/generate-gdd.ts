/**
 * Generate a Technical Game Design Document from a prompt + archetype.
 *
 * Ported verbatim from OpenGame/packages/core/src/tools/generate-gdd.ts.
 * Wrapper adapted: exposes a plain async function instead of a qwen
 * BaseDeclarativeTool. The LLM call goes through the LLMClient adapter so
 * callers pick their own provider (Claude via @swipi/core/llm#AnthropicLLMClient
 * is the default in swipi-engine; any LLMClient works).
 */
...
export interface GenerateGDDParams {
  /** User's raw game idea or description. */
  raw_user_requirement: string;
  /** Archetype determined by classifyGame(). */
  archetype: GameArchetype;
  /** Optional: summary of existing gameConfig.json if the agent already read it. */
  config_summary?: string;
}

export interface GenerateGDDOptions {
  llm: LLMClient;
  /**
   * Absolute path to the swipi docs directory containing:
   *   gdd/core.md
   *   modules/{archetype}/design_rules.md
   *   modules/{archetype}/template_api.md
   * In swipi-engine this is typically packages/shared/docs. If not supplied,
   * generate-gdd falls back to walking up from process.cwd() looking for `docs`.
   */
  docsDir?: string;
  signal?: AbortSignal;
...
class GenerateGDDInvocation {
  constructor(
    private readonly params: GenerateGDDParams,
    private readonly options: GenerateGDDOptions,
  ) {}

  async execute(): Promise<string> {
    const systemPrompt = await this.buildSystemPrompt();
    const userPrompt = this.buildUserPrompt();
    return this.callModel(systemPrompt, userPrompt);
  }
  ...
  private async buildSystemPrompt(): Promise<string> {
    ...
    // Prefer the caller-supplied docsDir; fall back to walking up from cwd.
    let docsDir = this.options.docsDir ?? '';
    if (!docsDir) {
...
- Wave clear bonuses: 10-100g per wave`,
  ...
  private async callModel(
    ...
    const response = await this.options.llm.complete({
      ...
      tier: this.options.tier ?? 'balanced',
      temperature: 0.7,
      maxTokens: 10000,
...
/**
 * Generate a Technical Game Design Document for a classified game.
 *
 * @example
 * const gdd = await generateGDD(
 *   { raw_user_requirement: "A Snake clone with dark theme", archetype: "grid_logic" },
 *   { llm: new AnthropicLLMClient(), docsDir: path.join(sharedRoot, 'docs') }
 * );
 * await fs.writeFile('GAME_DESIGN.md', gdd);
 */
export async function generateGDD(
  params: GenerateGDDParams,
  options: GenerateGDDOptions,
): Promise<string> {
  const invocation = new GenerateGDDInvocation(params, options);
...
/**
 * Validate parameters before calling generateGDD. Returns null if OK,
 * else a human-readable error message. Exported for use by upstream layers
 * (e.g., REST API request validation).
 */
export function validateGenerateGDDParams(
  params: Partial<GenerateGDDParams>,
): string | null {
  if (!params.raw_user_requirement || params.raw_user_requirement.trim() === '') {
    return 'raw_user_requirement must be a non-empty string';
  }
  if (!params.archetype) {
    return 'archetype is required. Run classifyGame first.';
  }
  if (!VALID_ARCHETYPES.includes(params.archetype)) {
    return `Invalid archetype: ${params.archetype}. Must be one of: ${VALID_ARCHETYPES.join(', ')}`;
  }
  return null;
}

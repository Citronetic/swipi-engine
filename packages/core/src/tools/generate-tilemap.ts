/**
 * Deterministic ASCII → Phaser Tilemap JSON generator with 47-tile blob auto-tiling.
 *
 * Ported verbatim from OpenGame/packages/core/src/tools/generate-tilemap.ts +
 * services/auto-tiler.ts. No LLM calls — pure algorithmic conversion. The
 * qwen BaseDeclarativeTool wrapper is replaced with a plain `generateTilemap`
 * function that takes an explicit `workspaceDir` instead of reading a Config.
 */

import { AutoTiler } from '../utils/auto-tiler.js';
import * as fs from 'fs/promises';
import * as path from 'path';

export interface GenerateTilemapResult {
  /** Absolute paths of the tilemap JSON files written. */
  generatedMaps: string[];
  /** Optional object-spawn info per map (if `object_markers` was supplied). */
  objects: Array<{ mapKey: string; objects: string[] }>;
  /** Human-readable summary (same string OpenGame surfaced as llmContent). */
  summary: string;
  /** Non-fatal errors from individual maps that failed. */
  errors: string[];
}
...
class GenerateTilemapInvocation {
  constructor(
    private readonly workspaceDir: string,
    private readonly params: GenerateTilemapParams,
  ) {}
  ...
  async execute(): Promise<GenerateTilemapResult> {
    const targetDirName =
      this.params.output_dir_name || path.join('public', 'assets');
    const absoluteAssetsDir = path.join(this.workspaceDir, targetDirName);
    ...
    if (this.params.maps && this.params.maps.length > 0) {
      ...
      throw new Error(
        'Either `maps[]` array or legacy `map_key`+`layout_ascii` parameters must be provided.',
      );
    }
    ...
    if (results.length === 0) {
      throw new Error(`Failed to generate any map: ${errors.join('; ')}`);
    }
...
`;
    ...
    return {
      generatedMaps: results.map((k) =>
        path.join(absoluteAssetsDir, `${k}.json`),
      ),
      objects: allObjects,
      summary: instruction,
      errors,
...
export interface GenerateTilemapOptions {
  /**
   * Absolute path to the project directory whose `public/assets/` will
   * receive the generated tilemaps and asset-pack entries.
   */
  workspaceDir: string;
}

/**
 * Convert ASCII layouts into Phaser Tilemap JSON files with optional
 * 47-tile blob auto-tiling. Writes to `<workspaceDir>/<output_dir_name>/`
 * (default `public/assets/`) and updates `asset-pack.json` in-place.
 *
 * @example
 * const result = await generateTilemap(
 *   {
 *     tileset_key: 'dungeon_floor',
 *     mode: 'floor',
 *     maps: [{ map_key: 'level_1', layout_ascii: ['####','#..#','####'], legend: {} }],
 *   },
 *   { workspaceDir: '/path/to/game-project' },
 * );
 */
export async function generateTilemap(
  params: GenerateTilemapParams,
  options: GenerateTilemapOptions,
): Promise<GenerateTilemapResult> {
  if (!params.tileset_key) {
    throw new Error('`tileset_key` is required.');
  }
  const invocation = new GenerateTilemapInvocation(
